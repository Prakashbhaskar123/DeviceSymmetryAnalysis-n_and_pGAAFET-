...
if __name__ == "__main__":
    ...
    plot_iv(df_n70, "drain OuterVoltage", "drain TotalCurrent",
            label="n70 (Vg = -0.5 V)", ax=ax)
    plot_iv(df_n614, "drain OuterVoltage", "drain TotalCurrent",
            label="n614 (Vg = +0.5 V)", ax=ax)
 
    ax.set_title("Id-Vd comparison: n70 vs n614")
    ax.axhline(0, color='gray', linewidth=0.8)
    ax.axvline(0, color='gray', linewidth=0.8)
    ax.legend()
 
    fig.tight_layout()
    fig.savefig("single_plots.png", dpi=200)
    print("Saved demo_plots_1.png")
